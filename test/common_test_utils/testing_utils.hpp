...
{
   return std::ranges::all_of(std::views::zip(rng1, rng2), [](const auto& v_w) {
      const auto& [v, w] = v_w;
      return v == w;
   });
}
...
{
   return std::ranges::all_of(rng1, [&](const auto& v) { return std::ranges::contains(rng2, v); })
          and std::ranges::all_of(rng2, [&](const auto& v) {
                 return std::ranges::contains(rng1, v);
              });
}
