# This file is managed by Conan, contents will be overwritten. To keep your changes, remove these comment lines, but the
# plugin won't be able to modify your requirements
    ...
    if(CMAKE_SYSTEM_NAME
       AND NOT
        ...
        if(CMAKE_SYSTEM_NAME MATCHES "Darwin|iOS|tvOS|watchOS")
            # CMAKE_OSX_SYSROOT contains the full path to the SDK for MakeFile/Ninja generators, but just has the
            # original input string for Xcode.
...
function(detect_arch ARCH)
    # CMAKE_OSX_ARCHITECTURES can contain multiple architectures, but Conan only supports one. Therefore this code only
    # finds one. If the recipes support multiple architectures, the build will work. Otherwise, there will be a linker
    # error for the missing architecture(s).
...
macro(detect_gnu_libstdcxx)
    ...
    int main () {}"
        _CONAN_GNU_LIBSTDCXX_IS_CXX11_ABI)
...
function(detect_lib_cxx OS LIB_CXX)
    ...
    elseif(CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
        # Do nothing - compiler.runtime and compiler.runtime_type should be handled separately:
        # https://github.com/conan-io/cmake-conan/pull/516
        return()
    else()
        ...
        if(CMAKE_MSVC_RUNTIME_LIBRARY)
            ...
            if(NOT
               ...
               _KNOWN_MSVC_RUNTIME_VALUES)
                message(
                    FATAL_ERROR
                        "CMake-Conan: unable to map MSVC runtime: ${CMAKE_MSVC_RUNTIME_LIBRARY} to Conan settings")
            endif()
            ...
            # Only define compiler.runtime_type when explicitly requested If a generator expression is used, let Conan
            # handle it conditional on build_type
...
function(detect_build_type BUILD_TYPE)
    ...
    if(NOT _MULTICONFIG_GENERATOR)
        # Only set when we know we are in a single-configuration generator Note: we may want to fail early if
...
function(detect_host_profile output_file)
    ...
    if(MYCOMPILER_VERSION)
        string(
            APPEND
            PROFILE
            compiler.version=${MYCOMPILER_VERSION}
            "\n")
    endif()
    if(MYCOMPILER_RUNTIME)
        string(
            APPEND
            PROFILE
            compiler.runtime=${MYCOMPILER_RUNTIME}
            "\n")
    endif()
    if(MYCOMPILER_RUNTIME_TYPE)
        string(
            APPEND
            PROFILE
            compiler.runtime_type=${MYCOMPILER_RUNTIME_TYPE}
            "\n")
    endif()
    if(MYCXX_STANDARD)
        string(
            APPEND
            PROFILE
            compiler.cppstd=${MYCXX_STANDARD}
            "\n")
    endif()
    if(MYLIB_CXX)
        string(
...
function(conan_install)
    ...
    if(NOT
       ...
       "0")
        ...
        # the files are generated in a folder that depends on the layout used, if one is specified, but we don't know a
        # priori where this is. TODO: this can be made more robust if Conan can provide this in the json output
        string(
            JSON
...
macro(conan_provide_dependency method package_name)
    ...
    # search order or search paths This follows the guidelines from the find_package documentation
    # (https://cmake.org/cmake/help/latest/command/find_package.html): find_package (<PackageName> PATHS paths...
    # NO_DEFAULT_PATH) find_package (<PackageName>)
    ...
    # Invoke find_package a second time - if the first call succeeded, this will simply reuse the result. If not, fall
    # back to CMake default search behaviour, also allowing modules to be searched.
...
macro(conan_provide_dependency_check)
    ...
    if(NOT _CONAN_PROVIDE_DEPENDENCY_INVOKED)
        message(WARNING "Conan is correctly configured as dependency provider, "
                        "but Conan has not been invoked. Please add at least one " "call to `find_package()`.")
...
# Add a deferred call at the end of processing the top-level directory to check if the dependency provider was invoked
# at all.
cmake_language(
    DEFER
    DIRECTORY
    "${CMAKE_SOURCE_DIR}"
    CALL
    conan_provide_dependency_check)
